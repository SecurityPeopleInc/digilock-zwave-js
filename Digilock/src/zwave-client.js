...
export class ZWaveProvisioningClient extends EventEmitter {
		...
		try {
			...
			if (
				...
				if (Object.keys(validKeys).length > 0) {
					...
					console.warn(
						`⚠️  No valid security keys found for standard Z-Wave`,
					);
				}
			} else {
				console.warn(
					`⚠️  No security keys provided for standard Z-Wave`,
				);
			}
			...
			if (
				...
				if (Object.keys(validKeys).length > 0) {
					...
					console.warn(
						`⚠️  No valid security keys found for Long Range`,
					);
				}
			} else {
	...
	setupControllerHandlers() {
		...
		controller.on("inclusion started", (strategy) => {
			...
			console.log(
				`[Smart Start]   Current state: ${stateName} (${currentState})`,
			);
			...
			if (strategy === 1 || strategyName === "SmartStart") {
				try {
					...
					console.log(
						`[Smart Start]   Active provisioning entries that could trigger inclusion: ${activeEntries.length}`,
					);
				} catch (err) {
					console.warn(
						`[Smart Start]   Could not get provisioning entries: ${err.message}`,
					);
				}
			}
		});
		...
		controller.on("inclusion failed", () => {
			...
			try {
				...
				const activeEntries = entries.filter((e) => e.status === 0); // 0 = Active
				console.error(
					`[Smart Start]   Active provisioning entries: ${activeEntries.length}`,
				);
				...
				console.error(
					`[Smart Start]   Error getting provisioning entries: ${err.message}`,
				);
			}
		});
	...
	async provisionSmartStartNode(entry) {
		...
		const verifyEntry = this.driver.controller.getProvisioningEntry(
			entry.dsk,
		);
		if (verifyEntry) {
			console.log(
	...
	/**
	 * Unprovisions all Smart Start provisioning entries for a given node.
	 * This is called automatically when a node is commissioned.
	 * @param {import("../../packages/zwave-js/src/Node.js").ZWaveNode} node
	 * @private
	 */
	_unprovisionNodeEntries(node) {
		...
		try {
			...
			// Get the node's DSK if available
			let nodeDSK = null;
			if (node.dsk) {
				try {
					nodeDSK = this.normalizeDSK(dskToString(node.dsk));
				} catch (error) {
					console.warn(
						`[Provisioning] Could not convert node ${node.id} DSK to string:`,
						error.message,
					);
				}
			}

			// Find all entries that match this node
			...
			for (const entry of allEntries) {
				...
				// Match by DSK if node has a DSK
				if (nodeDSK) {
					const normalizedEntryDSK = this.normalizeDSK(entry.dsk);
					if (normalizedEntryDSK === nodeDSK) {
						matchingEntries.push(entry);
					}
				}
			}

			// Unprovision all matching entries
			if (matchingEntries.length > 0) {
				console.log(
					`[Provisioning] Found ${matchingEntries.length} provisioning entry/entries for node ${node.id}, unprovisioning...`,
				);

				for (const entry of matchingEntries) {
					const normalizedDSK = this.normalizeDSK(entry.dsk);
					console.log(
						`[Provisioning] Unprovisioning entry with DSK: ${normalizedDSK} (nodeId: ${
							entry.nodeId || "none"
						})`,
					);
					controller.unprovisionSmartStartNode(normalizedDSK);
				}

				console.log(
					`[Provisioning] ✅ Successfully unprovisioned ${matchingEntries.length} entry/entries for node ${node.id}`,
				);
			} else {
				console.log(
			...
			console.error(
				`[Provisioning] Error unprovisioning entries for node ${node.id}:`,
				error,
			);
		}
	}
